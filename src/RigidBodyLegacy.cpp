...
RigidBodyLegacy::RigidBodyLegacy (GameObject& associated, float gravityValue): Component(associated) {
    // bodyType = DYNAMIC;
    gravityEnabled = true;
    ...
    type = ComponentType::_RigidBodyLegacy;
}

RigidBodyLegacy::~RigidBodyLegacy () {
    collidingOthers.clear();
}

void RigidBodyLegacy::Start () {
...
void RigidBodyLegacy::Update (float dt) {
    Vec2 position = associated.box.GetPosition();
...
void RigidBodyLegacy::HandleGravity (float dt) {
...
bool RigidBodyLegacy::IsGrounded () {
    return collidingFaces[DOWN];
}

Vec2 RigidBodyLegacy::GetVelocity () {
    return velocity;
}

void RigidBodyLegacy::Translate (Vec2 displacement) {
...
void RigidBodyLegacy::AddForce (Vec2 force) {
    velocity += force;
}

void RigidBodyLegacy::CancelForces (RigidBodyLegacy::Axis axis) {
    switch (axis) {
        case HORIZONTAL:
            velocity.x = 0.0f;
            break;
...
void RigidBodyLegacy::NotifyCollision (GameObject& other) {
    /*--------------------------------------------------------------------------------------------------*/
...
void RigidBodyLegacy::NotifyNoCollision (GameObject& other) {
    /*--------------------------------------------------------------------------------------------------*/
...
bool RigidBodyLegacy::IsColliding (ColliderFace face) {
    return collidingFaces[face];
}

void RigidBodyLegacy::CheckDeletedColliders () {
...
bool RigidBodyLegacy::Is (std::string type) {
    return (type == "RigidBodyLegacy");
}

bool RigidBodyLegacy::Is (ComponentType type) {
